...
final class DSDB {
    ...
    private let DB_FILE_NAME: String = "DSDB.sqlite"
    
    let apiKey: String
    
    init(token: String) {
        apiKey = String(token.unicodeScalars.filter({ CharacterSet.alphanumerics.contains($0) }))
        open()
    }
    ...
    private func pathToDb() -> String? {
        ...
        guard let urlUnwrapped = url?.appendingPathComponent(apiKey + "_" + DB_FILE_NAME).path else {
            return nil
        }
        return urlUnwrapped
    }
    ...
    private func tableNameFor(_ persistenceType: PersistenceType) -> String {
        return "datasenses_\(apiKey)_\(persistenceType)"
    }
    
    private func reconnect() {
        Logger.warn(message: "No database connection found. Calling DSDB.open()")
        open()
    }
                ...
                while sqlite3_step(selectStatement) == SQLITE_ROW {
                    if let blob = sqlite3_column_blob(selectStatement, 1) {
                        ...
                        if let jsonObject = JSONHandler.deserializeData(data) as? InternalProperties {
                            ...
                            entity[InternalKeys.dsEntityId] = id
                            rows.append(entity)
                        }
                        rowsRead += 1
                    } else {
